...
CRAFT では Director は直接ブロックを置かないため，第2層は一般的な物理タスク DAG ではなく，まず Builder の候補行動を扱う `Action Candidate DAG` として実装する．Minecraft 実環境へ拡張する段階では，この層を VillagerAgent の既存 Task DAG に接続する．

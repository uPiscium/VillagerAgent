...
The fixed set of historical diagnostics recorded before this policy is classified `legacy-diagnostic-unarchived`, with every expected run accounted as missing and `publication_satisfied` and `claim_eligible` set to false. This is only an explicit historical inventory: it does not satisfy publication, cannot support paper or performance claims, and cannot be assigned to new result IDs. Every new `benchmark-result` or `paper-result` declaration after adoption of this policy must resolve to a fetched, checksum-verified archive.

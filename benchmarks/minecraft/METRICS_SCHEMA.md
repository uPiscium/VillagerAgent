...
For paired real-runtime comparisons, `task_completion_rate` is the prespecified primary outcome and one judged task run is the comparison unit. Pair by `(task_id, seed, world_id)` so results from different reset worlds or world snapshots are not treated as matched. Dry runs and connectivity checks are diagnostic only. Other scalar metrics, including `progress`, are exploratory and must be analyzed in separate contracts. See `docs/benchmarks/paired_comparisons.md` and `benchmarks.common.analysis`.

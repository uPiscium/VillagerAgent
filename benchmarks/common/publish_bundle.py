...
_MINECRAFT_SLASH_COMMANDS = {
    ...
    "give", "item", "kill", "locate", "msg", "op", "place", "recipe",
    "setblock", "summon", "tellraw", "time", "tp", "weather",
}
...
_PATH_LIKE_KEY = re.compile(
    r"(?:^|_)(?:cwd|destination|dir|directory|executable|file|filename|home|path|root|workspace)s?$"
)
_COMMAND_LIKE_KEYS = {
    "action",
    "command",
...
def sanitize_local_paths(value: str, *, key: str | None = None) -> str:
    ...
    if _is_minecraft_command(stripped, key=key):
        return value
    sanitized = value
    ...
    sanitized = _CONTEXTUAL_SINGLE_SEGMENT_POSIX_PATH.sub(
        lambda match: match.group(0).replace(match.group(1), _LOCAL_PATH_MARKER),
        sanitized,
    )
    sanitized = _SINGLE_SEGMENT_POSIX_LINE.sub(
        lambda match: match.group(0)
        if _is_minecraft_command(match.group("path"), key=key)
...
def contains_local_absolute_path(value: str, *, key: str | None = None) -> bool:
    ...
    for pattern in _LOCAL_PATH_PATTERNS:
        if any(
            ...
            return True
    if _CONTEXTUAL_SINGLE_SEGMENT_POSIX_PATH.search(value):
        return True
    return any(
        not _is_minecraft_command(match.group("path"), key=key)
...
def _preserve_free_text_path_match(value: str, match: re.Match[str]) -> bool:
    ...
    return bool(
        ...
        or _is_minecraft_command(candidate, key=None)
    )


def _is_minecraft_command(value: str, *, key: str | None) -> bool:
    if _is_path_like_key(key):
        return False
    match = re.match(r"^/([A-Za-z]+)(?:\s|$)", value.strip())
    if not match or match.group(1).lower() not in _MINECRAFT_SLASH_COMMANDS:
        return False
    if key is None:
        return value.strip().startswith("/")
    normalized = key.lower().replace("-", "_")
    return normalized in _COMMAND_LIKE_KEYS
